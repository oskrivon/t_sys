...
**Задачи:**
- [ ] Чем отличаются март/апрель/май: реализованная вола, тренд/чоп BTC, funding, разброс по
      монетам (good-rate помесячно по каждой монете — спад во всех или в части?)
- [ ] Расширить вселенную за пределы 9 мем (другие активные / майоры) — spacing-селекция и
      деградация good-rate воспроизводятся, или это специфика мем-сегмента?
- [ ] Если деградация = режим: коррелирует ли good-rate месяца с BTC-волой/трендом → условие
      «когда сетап вообще работает»
