...
### Icebreaker ФЕЙД (отскок) — комплементарное расширение, гейт maker-fill (2026-07-01)

Найден maker-лид: фейдить строго mom<0 хвост (пробой при отрицательном 30m-моментуме = выдох), который правило-пробой выбрасывает. klines 2y/15монет: MAKER +0.037% WR59%, 21/24 мес+, мемы 8/8; taker мёртв. Структурно maker-исполним (контр-тренд) — впервые не упирается в fee-стену сходу. Детали → docs/ICEBREAKER_RESEARCH.md.
- [ ] **Гейт запущен:** `icebreaker_fade_fill.py` на серверной ленте (9 мемов × Feb-May) — реальны ли maker-филлы и не токсичны ли (gr_fill vs gr_miss). Снять результат.
- [ ] Если филлы ОК: 3y/41-монета apples-to-apples с сертифицированным пробоем; затем scorecard (DSR/CPCV/PBO/MinBTL) на mom<0 фейде — тонко (+0.037%), значимость под вопросом.
- [ ] Sweep stop-buf (0.003/0.005/0.008) и mom-порог на факт-филле; проверить fee-tier-чувствительность (maker 0.02% vs 0% ребейт).
- [ ] Если проходит → доклеить к пробою как второй режим (один детектор, оба хвоста mom: высокий→пробой taker, низкий→фейд maker, середину пропускаем); отдельный paper-трек или расширить существующий демон.
- [ ] Если фаллишься на провалах (gr_miss>gr_fill) → закрыть с числом, как Phase 1.7/1.8.

### Rate-limit'ы Bybit на ледоколе — обработка + возможно проксирование (2026-07-01)

**Симптом:** демон `run_icebreaker_paper.py` ловит `bybit retCode 10006 "Too many visits. Exceeded the API Rate Limit"` — ~433 warning'а за первые 17ч (спорадически, не каждый цикл). Опрос 41 монеты каждые ~20с последовательными `fetch_ohlcv` (~120 REST/мин), делит IP-лимит Bybit с движком (funding/VR) и, возможно, с другими процессами на том же хосте. Сейчас ошибка ловится в `except` → символ пропускается на цикл, добирается на следующем.

**Риск (низкий, но реальный):** если 10006 приходит ровно на минуте закрытия брейка по монете, свежий бар подберётся на следующем цикле, но `fresh_signal` проверяет `ts_close == last_ts + BAR_MS` — надо ПРОВЕРИТЬ, не теряется ли сигнал при пропуске цикла (в худшем случае — пропущенный вход). Мониторинг heartbeat уже показывает `errors`/цикл, так что частоту видно.

**Задачи:**
- [ ] Замерить реальную частоту 10006 по heartbeat (`errors`) за несколько дней — понять, косметика это или теряем сигналы. Проверить сценарий «rate-limit на баре закрытия брейка» на реплее (не пропадает ли вход).
- [ ] Дешёвые митигации без прокси: (а) поднять `poll_s` или добавить джиттер/паузу между символами (ccxt `enableRateLimit` уже вкл, но всплески проходят); (б) взвесить переход на **WS-стрим 1m-клайнов** вместо REST-поллинга (Bybit public WS `kline.1.<SYM>` — один коннект, без REST-лимита, снимает проблему в корне и снижает латентность входа); (в) отделить IP/аккаунт ледокола от движка.
- [ ] Если REST остаётся: **проксировать часть запросов** — раскидать 41 монету по нескольким исходящим IP (пул HTTP/SOCKS-прокси, у нас уже есть `proxies_static.txt` в tgcrawler на сервере как референс), балансировка по символам. Оценить стоимость/надёжность прокси vs выигрыш; WS скорее всего проще и лучше.
- [ ] Решение: WS-стрим (предпочтительно) ИЛИ REST+прокси-пул; задокументировать в RUNBOOK, редеплой демона. Связь с [[project_icebreaker_dataset]] (детектор/exit трогать нельзя — только слой добычи баров).

### Icebreaker paper — чек-ин по логу (с ~2026-07-14)

Демон `run_icebreaker_paper.py` запущен 2026-06-30, копит ~9 сигналов/мес. Свериться с
`/root/trading/data/icebreaker_paper_trades.jsonl` примерно через 2 недели (≈2026-07-14),
дальше периодически до ~10-20 закрытых сделок (~6-8 недель).
- [ ] Проверить, что демон жив (лог растёт, `icebreaker_paper.log` без ошибок; @reboot отработал при ребуте)
- [ ] Когда наберётся ≥10 закрытых сделок: посчитать live net/WR и сравнить с валидированным
      +0.08-0.11% и tape +0.083%. Фокус — слиппедж/частичные филлы на тонких щитках (прайор haircut ~10bps)
- [ ] Если live net держится в плюсе и близко к tape → кандидат на малый real-money; если проседает
      ниже tape → измерить, сколько съел слиппедж, и решить (фи-тир/sizing/только ликвидные монеты)
- [ ] Опц.: добавить crash-watchdog (сейчас только @reboot — переживает ребут, но не смерть процесса)

### VIP-пушер: дешёвый генератор объёма для апа уровня аккаунта Binance (2026-06-30)

**Мотивация:** анализ экономики ледокола показал — VIP2-3 поднимают его annual +5-7pp (Sharpe 1.5→2.0-2.2),
НО сам ледокол генерит лишь ~$0.5-1M оборота/мес при нужных VIP1 ~$10-15M/мес → до тиров не дотянуть его
собственным объёмом. Идея: отдельная **надёжная high-Sharpe / near-breakeven стратегия как генератор оборота**,
чья задача — не прибыль, а дешёвый/безубыточный объём для апа VIP, который затем удешевляет фи ВСЕГО портфеля
(ледокол + Weekend + funding). В позиции ледокол <1% времени → капитал свободен под это (особенно если ледокол
на плече 3-5× освобождает маржу).

**Кандидат из архива:** `docs/archive/SPREAD_TRADING_RESEARCH.md` — calendar/dynamic-basis z-score спред,
**gross Sharpe 12.4 (ETH Q2, WR 100%, edge ~0.05%/трейд)**, но NET-минус после funding на perp-ноге
(−0.05…−0.50% по DTE) → как профит-стратегия DEAD. ИМЕННО ЭТОТ профиль (огромный Sharpe, микро-доходность,
почти безубыток) идеален под volume-pumping: цель — net ≈ 0, а не плюс.

**Задачи:**
- [ ] Переоптимизировать спред-стратегию под цель «макс объём при net-косте ≈ 0» (а не макс PnL): подобрать
      пары/DTE/z-пороги, где funding+fees минимальны; рассмотреть maker-исполнение (ребейт обнуляет косты).
- [ ] Посчитать unit-экономику пампа: $ оборота/мес → достижимый VIP-тир (Binance 30d vol + BNB-баланс) vs
      net-кост пампа; сравнить с суммарной экономией фи по всему портфелю. Пампить стоит ТОЛЬКО если экономия
      по портфелю > кост пампа (для одного ледокола ($20-40k/г потолок) почти наверняка НЕ окупится — нужен
      крупный профитный объём, который тир удешевляет).
- [ ] Альтернатива спреду: delta-neutral maker (long perp / short perp кросс-биржа, либо grid вокруг mid) —
      возможно чище по косту объёма. Сравнить с переоптимизированным спредом.
- [ ] Связь с [[project_icebreaker_dataset]]: после получения VIP пересчитать annual ледокола на новом фи-тире.

### Icebreaker: ФИНАЛЬНЫЙ РАУНД → НАЙДЕН ЭДЖ, нужна валидация (2026-06-29)

**РЕЗУЛЬТАТ: закрытие было преждевременным — есть живой СЕЛЕКЦИОННЫЙ эдж.** Обе идеи заказчика
дали результат. mom-k0 (моментум пробойной свечи, известен на закрытии = в момент входа, без
lookahead) МОНОТОННО упорядочивает realized PnL 4/4 месяца. **CLEAN (one_sided≥0.9 & spacing>60)
× top-10–15% mom → вход маркетом на закрытии пробоя → managed exit = net-плюс 4/4 месяца включая
OOS-февраль** (top-15%: net +0.091% / gross +0.201% / WR 57% / 4-4 мес+; top-10%: net +0.190% /
gross +0.300% / WR 68% / 8-9 монет, n=65). gross 2–3× кост VIP0. Жёсткий отбор вылечил
регимо-зависимость (Apr/May → плюс). Субминутный вход ОПРОВЕРГНУТ (фейки дороже лучшей цены).
Детали: `docs/ICEBREAKER_RESEARCH.md` → «Финальный раунд». **Кандидат на валидацию, НЕ задеплоен.**

**ВАЛИДАЦИЯ ВЫПОЛНЕНА (2026-06-30) — 7/7, PAPER-READY:**
- [x] Ось найдена: **сила пробоя (абсолютный mom-k0), коин-агностично** (не вола: vol-Pearson −0.01; не атрибут монеты). Перцентиль → **фикс-порог** (mombin монотонен).
- [x] Широкая вселенная (40 монет) + 12 мес klines: эдж концентрирован в форсированных пробоях, не в «мем-кармане» (артефакт перцентиля).
- [x] Scorecard 2 года (klines −5bps haircut): **mom≥2.5% = 7/7 PASS при n_trials=1** (CPCV 1.84/P(>0)96%, DSR 0.001, MinBTL pass, PBO 0, multi-regime), net +0.112%/сделку WR 61%. При n=6 — 5/7 на волосок (DSR 0.052, MinBTL 1.985 vs 2.1г).

**ОТЛОЖЕНО (ПАУЗА по решению заказчика):**
- [x] **3-й год истории (klines 2023-06..2024-06) → БЕЗУСЛОВНЫЙ 7/7 (2026-06-30).** Дофетчен
      год, `klines_3y` = 2023-06-01→2026-06-01 (41 монета, 1000SHIB пуст). Momdump 118.5k сетапов
      → scorecard mom≥2.5% −5bps: **7/7 при n_trials=1 И n=6.** 3-й год закрыл обе слабые gates:
      DSR 0.052→**0.028**, MinBTL 1.985→**2.960** (vs 2.1г). 519 сделок, net +0.106%/сделку, WR 59%,
      сумма +54.9%, CPCV Sharpe 1.93, PBO 0, multi-regime 2. Дамп `data/ib_momdump_3y.jsonl`,
      стор `data/klines_3y`.
- [x] **Tape-кросс-чек правила mom≥2.5% (2026-06-30) — ЭДЖ ПОДТВЕРЖДЁН НА ЛЕНТЕ, но haircut был занижен.**
      `icebreaker_tape_xcheck.py`: фикс-правило на РЕАЛЬНОЙ ленте (9 мемов × фев-май) vs klines на
      тех же монетах/месяцах. (1) **Лента строго в плюсе:** mom≥2.5% net +0.427%/трейд, WR 93%,
      7/7 монет, 3/4 мес+ — правило живёт без klines-допущения. (2) **НО klines-оптимизм > заложенного
      haircut:** gap +0.10pp на 2.5% (растёт до +0.21pp на 3.0%), оптимизм концентрируется в high-mom
      хвосте, который и отбираем; scorecard закладывал лишь +0.05pp. (3) Состав 519 сделок: 45% мемы /
      55% majors (обе в плюсе). При честном дифф-haircut (10bps мемам / 5bps majors) **scorecard
      n=6 → 6/7, DSR падает** (p=0.120, Sharpe 1.49); net +0.083%/трейд, CPCV 1.43/P(>0)96%, MinBTL
      2.96, PBO 0 — крепкие. **n=1 устойчив 7/7 даже при 10bps** (DSR 0.038). ИТОГ: эдж реальный и
      tape-положительный, но сертификация чувствительна к haircut — чистый 7/7 только при оптимистичном
      5bps; при tape-реалистичных 6-10bps это 6/7 (DSR на грани) при строгом n=6. Paper-ready по планке
      проекта (Weekend пошёл в paper при 6/7), но НЕ безусловный. Дамп `data/ib_momdump_3y_diffhc.jsonl`.
- [~] **Paper trading — ЗАПУЩЕН LIVE на сервере (2026-06-30).** `scripts/run_icebreaker_paper.py` —
      демон валидированного правила (детектор импортирован дословно; exit — инкрементальная копия
      `simulate_exit_bars`, replay-гейт 0/11035 расхождений). Live: опрос Bybit 1m по 41 монате каждые
      ~20с, рыночный вход на закрытии пробоя, managed-exit по закрытым барам, лог gross+net@VIP0,
      resume из state. Без реальных ордеров. Крутится на серваке (`/root/trading_ib`, setsid + @reboot
      cron), лог `/root/trading/data/icebreaker_paper_trades.jsonl`, ~сотни MB RAM. **СБОР ДАННЫХ:** ждём
      ~10-20 сигналов (≈9/мес на 41 монету → ~6-8 недель), сравнить live net vs валидированный +0.08-0.11%
      и vs tape +0.083% (особый фокус — слиппедж/частичные филлы на щитках, прайор haircut ≈10bps).
      Открыто: чувствительность к фи-тиру/sizing/частоте; durability — добавить crash-watchdog (сейчас
      только @reboot).

**Решение (историческое):** ледокол считался закрытым по всем фронтам; финальный раунд это опроверг — найден живой кросс-валидированный эдж (см. выше).

**Шаг 1 — добить идеи заказчика.**

- [ ] **(1) Дискриминатор «жирных винеров» по широкому спектру.** Отобрать топ-N самых жирных
      раннеров (по MFE, экстремальный хвост — НЕ good@1%, а top-5/10%) и сравнить с остальными по
      максимально широкому набору: моментум (pre-break velocity, kalman-velocity на подходе), тренд
      (BTC-тренд + тренд монеты, EMA-стек), ликвидность (liq_ahead_rel, book-void впереди, OI),
      режим (реализованная вола монеты/рынка), время суток / сессия (US/Asia/EU), день недели,
      расстояние до соседних уровней, объём на пробое, возраст/spacing уровня. Цель — есть ли у
      ЭКСТРЕМАЛЬНОГО хвоста сепарабельность, которой нет у good@1%.
      *Прайор/связь:* Фаза 2 мультиварёный logreg+GBT над 22 фичами дал CV-AUC 0.49-0.51 для good И
      win — но на пороге good@1%, НЕ на экстремальном хвосте. Хвост маленький (n мал → риск оверфита),
      зато именно его берёт дискреционный скальпер. Данные есть: мажор-дампы (mfe) + лента + OI +
      liqmap + kalman. Дёшево (локально/частично сервер).
- [ ] **(2) Кривая скорости входа в пробой.** Определить окно: насколько быстро надо зайти, чтобы
      (а) уже быть уверенным что это пробой, но (б) не опоздать к разбору ликвидности. Для real vs
      fakeout построить явную кривую delay→{confidence, оставшаяся доля хода, realized edge} по
      сетке задержек (0/1/2/5/10/20с после пробоя). Где confidence растёт быстрее, чем уходит
      ликвидность — оптимум (если он есть выше безубытка).
      *Прайор/связь:* Фаза 2д (`icebreaker_confirm_sweep.py`, hold×delta) уже показала тренд:
      подтверждение поднимает WR (19→47%), но give-back раздувает avgLoss → expectancy − в КАЖДОЙ
      ячейке. Идея (2) — то же явление под углом «скорость/цена tradeoff», явной кривой. Честно:
      прайор отрицательный (мы уже видели, что give-back съедает), но кривая даст чистую
      артикуляцию «окна» и закроет вопрос с цифрами. Reuse `confirm_sweep` + tape.

**Edge-vs-cost гейт (правило фонда):** обе идеи — про СЕЛЕКЦИЮ/ТАЙМИНГ входа, т.е. бьются в ту же
стену «good price XOR good signal». Перед полным прогоном — прикидка на салфетке: даже если хвост
сепарабелен, конвертится ли он в gross > 2× косты (0.11%)? Если селекция опять живёт в excursion,
а не в извлекаемом PnL (как Фазы 1.5/1.9) — не открывать полный прогон.

**Шаг 2 — финальная валидация перед архивом (когда идей не осталось):**
- [ ] Свести единый итоговый вердикт в шапку `ICEBREAKER_RESEARCH.md` (все фазы 0-5 + экономика
      исполнения): эдж суб-комиссионный/исполнительный, не сигнальный
- [ ] Прогнать unit-тесты новых скриптов (`icebreaker_{regime,volnorm,major_klines,runner_exit,
      retest_flow}.py`) — хотя бы smoke, чтобы дамп воспроизводился
- [ ] Зафиксировать переиспользуемые активы (spacing-детектор `detect_major_breakouts`, лента 9 монет
      × 3 мес на сервере, liqmap, kalman-velocity, free-data источники Bybit/KuCoin)

**Шаг 3 — архив (по CLAUDE.md routing):**
- [ ] `docs/ICEBREAKER_RESEARCH.md` → `docs/archive/`
- [ ] Однострочный вывод в `docs/RESEARCH.md` → Архив
- [ ] Пометить связанные TODO-блоки ледокола как закрытые

### Icebreaker Фаза 5: канонический playbook скальпера (РЕТЕСТ + flow) (2026-06-29) — ЗАКРЫТО, adverse selection измерен

**Контекст:** веб-ресёрч показал — скальперы входят на РЕТЕСТЕ пробитого уровня + order-flow
подтверждение (не на уровне). Прогнали точную связку end-to-end (`icebreaker_retest_flow.py`).

**Результат (3 мес, taker):** adverse selection брутальный 3/3 мес — ~85-90% пробоев дают ретест
и это ДУДЫ (good@1% 11-16%), а раннеры (good 54-74%, ход 1.0-1.3%) ретеста НЕ дают, улетают.
retest_only −0.117/−0.126/−0.143%; CLEAN+flow лучшее −0.077/−0.078/−0.118%, минус везде. Канонический
метод скальпера, честно прогнанный, СТРУКТУРНО отбирает лузеров. Эдж скальпера = слой исполнения
(fee-tier/maker-rebate/sub-ms), вне нашего стека. **Ледокол закрыт со всех сторон, включая «как
делают люди».** Детали: `docs/ICEBREAKER_RESEARCH.md` Фаза 5.

### Icebreaker Фаза 4: runner-capture на clean-популяции (2026-06-28) — ЗАКРЫТО, суб-комиссионно

**Контекст (запрос заказчика):** входы Фазы 2 тестировались на СТАРОЙ грязной популяции, ДО
spacing-clean (Фаза 3). Пере-прогон дизайна «market-вход на пробое → 50%@+0.6%→БУ → ride» НА
clean-популяции, ride двумя выходами: фикс-трейл vs TIB-истощение (`icebreaker_runner_exit.py`).

**Результат:** гипотеза заказчика ВЕРНА — clean улучшает (ALL −0.118 → CLEAN −0.090 трейл,
WR 34→38%); TIB добавляет (CLEAN/tib −0.081, заметнее в мае). НО gross ≈ +0.03% < taker-косты
0.11% ⇒ net −0.081%/сделку, суб-комиссионно. 54% стопов даже на clean (вход на спайк-топе).
Улучшение +0.037пп не закрывает косты ×4. Детали: `docs/ICEBREAKER_RESEARCH.md` Фаза 4.

- [ ] (опц., низкий EV) book-density выход (L2, FART/WIF/PEPE март) — gross надо ×4, TIB дал +0.009

### Icebreaker: чем отличаются месяцы? + другие монеты (2026-06-28)

**Контекст:** пере-валидация качества уровня на 3 месяцах (март/апрель/май, 9 мем-монет) дала
spacing-селекцию `clean = one_sided≥0.9 & spacing>60` — робастный **+4пп хитрейта 3/3 мес**
(good15 17.8/13.9/14.2% vs база 12.4/10.2/10.3%), но НЕ торгуемо (exit OOS минус). Конъюнкция
`clean × liq-void` оказалась 2-месячным миражом — good-rate 23/23/**13.6%** (май сломал).
Детали: `docs/ICEBREAKER_RESEARCH.md` Фаза 3.

**Подозрение (заказчик):** спад good-rate конъюнкции 23→23→13.6% И базы 12.4→10.2→10.3% от
марта к маю выглядит как смена режима, а не шум. Не закрываем, пока не поймём причину.

**РАЗОБРАНО (2026-06-28): был vol-артефакт, не режим.** `icebreaker_regime.py` +
`icebreaker_volnorm.py`. good-rate ≈ функция волы монеты (Pearson 27 ячеек +0.61). Под
vol-нормированным `good` (MFE/вола≥k) спад схлопывается 12.4→10.2→10.3 ⇒ 11.8→10.1→11.2
(плоско); 2 монеты, что росли против тренда (ORDI/WLD) — те, чья вола росла. Spacing-clean
lift ВЫЖИВАЕТ vol-нормировку (+4.3/+3.1/+4.8пп 3/3 мес) ⇒ реальный vol-независимый признак.
Детали: `docs/ICEBREAKER_RESEARCH.md` Фаза 3 → «Почему различаются месяцы».

**Задачи:**
- [x] Чем отличаются март/апрель/май: реализованная вола, разброс по монетам — РАЗОБРАНО,
      good-rate следует за волой, спад = остывание волы мем-сегмента (FARTCOIN 17.5→11.2)
- [x] Если деградация = режим: коррелирует ли good-rate с волой — ДА, Pearson +0.61; vol-норм
      выравнивает месяцы ⇒ режимная загадка закрыта
- [x] Расширить вселенную за пределы 9 мем (майоры) — ВОСПРОИЗВЕЛОСЬ. 8 майоров (BTC/ETH/SOL/
      BNB/XRP/ADA/AVAX/LINK) × 3 мес, 1m klines, 1104 сетапа. Spacing-clean lift положителен
      6/6 ячеек (raw +2.7/+1.5/+7.8, vol-norm +1.6/+2.1/+9.3), пул +3.6пп ≈ мем +4пп ⇒
      cross-segment свойство, НЕ специфика мемов. Выборки малы (CI широкие), знак — главное
      доказательство. PnL-стена не двинулась (майоры низковолатильны, косты съедают сильнее).
      Скрипты: `icebreaker_fetch_ohlc.py` (1m OHLCV), `icebreaker_major_klines.py`.

### Icebreaker Фаза 2: level-cross entry — извлечь геометрию входа (2026-06-23)

**Контекст:** декомпозиция PnL + контрфакт входа (Фаза 1.9) нашли корень — всё съедает
**геометрия входа**. Вход на закрытии пробойной свечи (вершина спайка) = payoff≈1.0,
безубыток. Контрфакт «вход ПО УРОВНЮ» (fill-наивный потолок): **payoff~2.7, WR~70%,
+0.33%/сделку taker, хвост-устойчиво, реплицируется март→апрель**. Гейты считаются ДО
пробоя → уровень можно вооружить заранее и войти taker-ом на касании (fillable на
...
- [x] **(е) Angle 3 — динамика подхода + мультивариативный классификатор — DEAD (2026-06-24).**
  `icebreaker_approach.py` (3 монеты март, 1865 касаний; ответ на 3 вопроса по набору/стене/
  производной книги). Книга на лесенке `ts_cross−{30,15,10,5,2,0}с`: (Q1) производная глубины/стены
  на подходе AUC 0.444–0.541 (шум); (Q3) «MM держит стену→фейд» split на big-wall (n=622): снятие≥50%
  (n=30) good 0% vs устояла (n=592) good 4.2% vs база 4.0% — исход одинаков, стену снимают лишь 5%
  (footprint редкий И неинформативный); (Q2) **мультивариатив** logreg+GBT 5-fold CV над 22 фичами →
  **CV-AUC 0.49–0.51** vs good и win — никакая комбинация не разделяет. До пересечения фейк и реальный
  пробой микроструктурно идентичны.

- [ ] **(ж) → ЗАВТРА: визуальный разбор чартов (ручной аудит перед финальным закрытием).**
  Сгенерён сбалансированный набор `icebreaker_eyeball.py` — **24 чарта (12 раннеров + 12 фейков,
  3 монеты), локально `data/charts/eyeball/`** (2 панели: цена+уровень+консолидация+MFE/MAE сверху,
  resting-стена $ + cumulative eaten $ у уровня снизу). На двух открытых виден контраст: **раннер =
  стену СНЯЛИ** (стена→0 на пробое, eaten≈0), **фейк = стену ЕЛИ** (eaten растёт, рефиллы, абсорбция,
  разворот). Это скалярный `cancel:eat`/`absorbed_frac` (Фаза 1), который в агрегате был НЕ робастен —
  смотрим глазами, нет ли визуального гештальта, который скаляр размывает (форма схлопывания стены,
  тайминг рефиллов, поведение за 1–2 мин до касания). Опции: контактный лист 4×6; срез пограничных
  (MFE 0.3–0.8%); разметить «сняли/съели» вручную и пересчитать good-rate по этому признаку. Если и
  глаз ничего не видит — ледокол закрыт окончательно. Инфра: `icebreaker_eyeball.py`, кэш
  `tmp/ib_liq_empty.jsonl`, рендер `icebreaker_chart_setups.render`.

**ИТОГ (количественно): ледокол закрыт по входу.** Дискриминатора real/fakeout на касании нет нигде —
лента (0.50), стакан-снимок (0.50), динамика подхода + мультивариатив (CV-AUC 0.50), континуум
подтверждения (минус везде). Селекция вакуум-пробоя реальна (favorable-excursion, WR), извлекаемого
PnL под честным исполнением нет; единая суб-комиссионная/геометрическая стена. **Остался один ручной
шаг — визуальный разбор (ж);** если глаз пуст — финальное закрытие, дальше Miro / Weekend. Инфра вся в
`scripts/research/icebreaker_*.py`, дампы `tmp/ib_*`, сторы `data/{icebreaker_active,ib_book,oi,klines}`
на серве + `data/charts/eyeball/` локально.
...
**Контекст:** «стеночный» тезис мёртв по всем фронтам (стену снимают, кросс-венью lead=0),
НО механизм-заход нашёл первый OOS-валидный эдж — **пробой в позиционный/ликвидный вакуум**
(`liq_ahead_rel` низкий = пусто впереди → большой ход; гипотеза «каскад к топливу» инверсна).
Робастен кросс-коин, **реплицируется март→апрель**: good-rate 25→28%, maker realized
+0.11→+0.08%/сделку (медиана+, 8-9/9 монет), taker безубыток. Считается из бесплатных
OI+цена. Детали в `docs/ICEBREAKER_RESEARCH.md` Фаза 1.7.

**Задачи (по приоритету):**
- [x] **(а) Maker fill-risk / adverse-selection — решающий тест → ПРОВАЛЕН (2026-06-23).**
  `icebreaker_maker_fill.py`: пассивный лимит наливается только на честном касании ленты,
  развязка от фактического fill. Результат (март+апрель): offset 0 (= лимит на цене пробоя)
  воспроизводит +0.08–0.11%, но это маркетабельный ордер = taker по maker-фи, не maker.
  Реально-пассивный лимит (≥5bps) ловит **adverse selection** (gr_miss ~2× gr_fill на обоих
  месяцах — раннеры убегают, наливают фейлы); реалистичный maker-entry/taker-exit =
  безубыток-до-минуса, медиана отрицательна, ≤4/9 монет. **Эдж был артефактом допущения о
  гарантированном fill → ледокол закрыт.** Детали: `docs/ICEBREAKER_RESEARCH.md` Фаза 1.8.
  Подветки (б) больше OOS / (в) стакинг с OI — сняты (нечего вытягивать в плюс).
- [ ] **(б) Больше OOS — месяцы/монеты.** Сейчас 2 месяца (март+апрель), 9 монет.
  Прогнать ещё месяц (февраль / май) и/или расширить юниверс с теми же фикс-параметрами.
- [ ] **(в) Стакинг с OI-surge / book-void.** Утолщают ли независимые сигналы taker-маржу
  (сейчас taker безубыток)? Пересечь empty-ahead × OI-surge × thin-book, проверить realized.

**Данные/инфра готовы:** `icebreaker_liqmap.py` (+`fetch_oi/klines`), дампы `data/ib_liqmap{,_apr}.jsonl`,
`ib_liq_empty{,_apr}.jsonl`, OI/klines март+апрель локально, апрельский gated-стор +
`tmp/ib_gated_apr_*.jsonl` на серве. Фикс-параметры: порог liq_ahead_rel≤0.001926, exit
buf0.1/tp0.6/tr0.4/h30. Worktree `/root/trading_ib`, book-стор `data/ib_book` (FARTCOIN/WIF/PEPE),
KuCoin lv50 `tmp/kc` — переиспользуемы.

### Icebreaker: вход на РЕТЕСТ уровня (вместо входа на пробое) — 2026-06-21

**Контекст:** надёжный детектор сетапов доведён (`src/strategy/robust_levels.py`:
volume-profile + TTM-squeeze + quality score; ~30 тестов). Отбор по пересобранному
score (bounce×2 + squeeze + tightness-инверсия) даёт top-25% good-rate 17.3%
(AUC 0.597) — лучший за всё исследование. НО exit-sim всё равно минус на всех
тирах (meanR −0.31), потому что **вход на закрытии пробойной свечи = на вершине
спайка → 45% сделок выбивает по стопу** (1766/3958). Это последний непроверенный
рычаг и он бьёт прямо в проблему.

**Задача:** добавить альтернативный вход — НЕ на пробое, а на ретесте уровня после
пробоя (как в исходном описании заказчика: «или уйдёт на ретест»):
- [ ] В exit-sim/новом скрипте: после пробоя ждать возврат цены к уровню (±tol),
  входить ТАМ (не на спайке), стоп тесный за уровень, цель — продолжение
- [ ] Сравнить realized PnL/meanR: вход-на-пробое vs вход-на-ретесте, на top-score
  тире, активные монеты, taker+maker, горизонты 15/30м
- [ ] Если ретест даёт положительный meanR — нашли форму эджа, валидировать OOS
  (2-й месяц / больше монет); если нет — ледокол закрыт окончательно

**Данные/инфра готовы:** дамп `data/ib_robust_scored.jsonl`, стор ленты активных
монет `data/icebreaker_active` (9 монет, март), скрипты `icebreaker_robust.py` +
`icebreaker_exit_sim.py`. Детали в `docs/ICEBREAKER_RESEARCH.md`.
**На будущее:** рендерить графики из готового дампа, а не пере-детектить (сейчас
chart-режим зря гоняет всю детекцию ~12мин).
