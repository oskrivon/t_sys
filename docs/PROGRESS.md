...
**Итог Фазы 2:** на момент касания дискриминатора real/fakeout НЕТ ни в ленте, ни в стакане — 81% касаний фейк-поки, неотделимы в real-time. «Хорошая цена XOR хороший сигнал» фундаментально для входа-на-касании. Потолок +0.33% (идеальный fill) и floor −0.086% (raw armed-cross) известны. Единственный неисследованный честный рычаг — **континуум подтверждения** (вход после удержания пробоя Δ bps за Δt: give-back за отсев фейков). Новое: `icebreaker_book_gate.py` (reconstruct book@ts_cross±dt + AUC/gate-sweep), `ts_cross/side/level` в дампе `levelcross`. Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md) Фаза 2.
