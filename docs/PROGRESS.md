...
4. **Вердикт:** vision-гипотеза закрыта. Глаз реален, но эдж живёт в favorable-excursion, не в извлекаемом PnL (та же стена, что Фазы 1.5/1.8). Заноза: устойчиво плюсовая медиана при нулевом среднем → дальше разбираем, что именно съедает PnL. Инфра: lookahead-free render + **incremental dump/resume** (пережил жёсткий хост-стоп посреди прогона — VMware balloon/оверкоммит провайдера, тикет `docs/server_incident_2026-06-23_host_ticket.txt`). Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md) Фаза 1.9.
