...
**Доходность/Sharpe:** Sharpe ≈ **1.49** (daily ann√365, совпадает с per-trade и CPCV 1.43-1.93). Annual ≈ **+14.4%** (1× нотионал/трейд, без компаунда). VIP0 baseline.

**Потери:** WR 59%, avg win +0.60% / avg loss −0.67% (payoff 0.89 — это win-rate эдж). Худший трейд **−4.03%** нотионала, p1 −1.36%, p5 −0.90%. Макс серия убытков **6**. **Макс просадка −8.26%** нотионала (= капитала при 1× полном деплое). Помесячно mean +1.20% / std 2.89% / худший месяц −4.74% / 13 из 36 мес минусовые.

**Плечо:** нотионал капается ликвидностью (~$16k ср. / ~$100k пик при 6 одновр.), это фикс. Плечо снижает НУЖНУЮ маржу (5× → ~$20k вместо ~$100k), множит return-on-equity ×L И просадку ×L (1× DD −8% → 3× −25% → 5× −41%; худший трейд −4%×L). Абсолютный $-профит НЕ меняет (он ликвидностью-капнут). Разумный потолок ~3-5× из-за −4% мем-гэпа + серии 6. Освобождённый капитал → под VIP-пушер/другое.

**Ликвидность (потолок капитала):** 1-мин $-объём на пробойном баре: щитки медиана ~$354k (но FLOKI/JTO/LDO/POPCAT $75-150k), majors похоже; ликвидные DOGE/SOL/XRP/ETH $9-29M. Безопасный ордер @1% бар-объёма ≈ **$3.5k на медианной монете**. Сайзинг 1% (cap $200k): ср. позиция $16k, оборот ~$0.47M/мес, профит ~$1.6k/мес (~$19k/г); 2% (cap $500k): $35k, ~$1M/мес, ~$3.6k/мес (~$43k/г). **Абсолютный потолок ~$20-40k/год** на капитале ~$100-250k — это нишевый high-Sharpe движок на малый капитал, не масштабируемый.

**VIP-тиры:** апнуть до VIP2-3 даёт annual +14%→+20%, Sharpe 1.5→2.0-2.2 (fee_units=2 → каждый 1bp/сторону = +2bp/трейд). НО оборот стратегии ~$0.5-1M/мес vs нужные VIP1 ~$10M+ → **сама себя до VIP не дотянет** (в 10-50× мало). Фи-скидка применима, только если VIP уже есть от другой активности. → завёл TODO «VIP-пушер» (отдельный дешёвый генератор объёма из архивного спреда). Скрипты разбора: ad-hoc на ib_momdump_3y + klines_3y volume.

### 2026-06-30 — Icebreaker: tape-кросс-чек (#2) — эдж подтверждён на ленте, но сертификация чувствительна к haircut

Второй отложенный пункт: проверить deployed-правило `CLEAN & mom-k0≥2.5%` на РЕАЛЬНОЙ ленте, убрав klines-допущение (вход-на-закрытии-бара / выходы-по-барам), которое лежит в основе scorecard. Локально уже были tape-дампы 9 мемов × фев-май (`ib_momdump_{FEB,MAR,APR,MAY}.jsonl`, считаны `icebreaker_mom_gate.py` на серверной ленте). Написал `icebreaker_tape_xcheck.py` — apples-to-apples лента vs klines на тех же 9 монетах/месяцах.

**Результаты:**
1. **Лента подтверждает эдж:** mom≥2.5% на ленте = net **+0.427%/трейд, WR 93%, 7/7 монет+, 3/4 мес+** (n=14). Правило строго net-положительно и без klines-допущения. Помесячно: фев +0.553%, апр +0.225%, май +0.402%.
2. **НО klines-оптимизм больше заложенного haircut.** Gap (klines_net − tape_net): +0.068pp @2.0%, **+0.10pp @2.5%**, +0.21pp @3.0% — растёт с порогом, т.е. оптимизм сидит именно в high-mom хвосте, который мы и отбираем (бар-вход ловит спайк полнее реальной ленты). Scorecard закладывал лишь +0.05pp.
3. **Состав 519 klines-сделок:** 45% мемы (net +0.171%) / 55% majors (net +0.143%) — обе группы в плюсе. Gap +0.10pp замерен на мемах; на majors (ниже вола) почти наверняка меньше, но major-ленты нет (потому и klines).
4. **Чувствительность сертификации к haircut** (3-летка, n=6): 5bps→7/7 (DSR 0.028); 6bps→6/7 (DSR 0.056); 8bps→6/7 (0.17); 10bps→6/7 (0.38). DSR ломается между 5 и 6bps. Дифф-haircut (10bps мемам / 5bps majors) → **n=6: 6/7**, DSR p=0.120 (Sharpe 1.49); net +0.083%/трейд, CPCV 1.43/P(>0)96%, MinBTL 2.96, PBO 0 — остальное крепко. **n=1 устойчив 7/7 даже при 10bps** (DSR 0.038).

**Вывод:** эдж реальный и tape-положительный — главное опровержение «excursion-not-PnL» держится на честной ленте. Но безусловный 7/7 из #1 был артефактом оптимистичного 5bps haircut; при tape-реалистичных 6-10bps и строгом n=6 (штраф за поиск по сетке 6 порогов) рушится тест значимости DSR — это 6/7. CPCV/MinBTL/PBO/multi-regime неизменно сильны → эдж робастно положителен в кросс-валидации, тонок по значимости. Paper-ready по планке проекта (Weekend → paper при 6/7), но осторожнее: для #3 закладывать слиппедж/haircut ближе к 10bps. Скрипт `icebreaker_tape_xcheck.py`, дамп `ib_momdump_3y_diffhc.jsonl`.

### 2026-06-30 — Icebreaker: 3-й год истории → БЕЗУСЛОВНЫЙ 7/7 (отложенный пункт #1 закрыт)

Возобновили ледокол после паузы. Первый из 3 отложенных пунктов — добор 3-го года данных, чтобы убрать «на волосок» при честном n_trials=6 (было 5/7: DSR 0.052, MinBTL 1.985 vs наши 2.1г).

**Что сделано:** дофетчен год 1m-OHLC (2023-06-01→2024-06-01) по всем 42 символам вселенной (4 параллельных группы, Bybit free kline), смержен с `klines_2y` → **`klines_3y` = 2023-06-01→2026-06-01, ровно 3 года** (BTC 1.58M баров; FARTCOIN/POPCAT/ENA/TAO и др. с реальной даты листинга; 1000SHIB пуст и в 2-летке — эффективно 41 монета). Регенерирован momdump (4 группы) → `data/ib_momdump_3y.jsonl`, 118.5k сетапов, 36 месяцев покрытия.

**Результат scorecard** (правило CLEAN & mom-k0≥2.5%, −5bps haircut, 519 сделок, net +0.106%/сделку, WR 59%, сумма +54.9%, 39 монет):

| n_trials | CPCV Sharpe | DSR p | MinBTL | PBO | Multi-regime | Итог |
|---|---|---|---|---|---|---|
| 1 | 1.93 | 0.000 | 2.96 vs 1.0г | 0 | 2 | **7/7** |
| 6 | 1.93 | **0.028** | **2.96** vs 2.1г | 0 | 2 | **7/7** |

**3-й год закрыл обе слабые gates** (DSR 0.052→0.028, MinBTL 1.985→2.960). Эдж теперь **безусловно 7/7 при честном счёте проб**, не «на волосок». Метрики net/WR консистентны с 2-леткой (+0.112%/61%). Осталось 2 отложенных пункта: tape-кросс-чек (убрать klines-допущение) и paper trading. Скрипты: `icebreaker_{fetch_ohlc,merge_klines,mom_gate_kl,scorecard}.py`.

### 2026-06-24 — Icebreaker Фаза 2 Angle 2: L2-книга на касании — DEAD, дискриминатора real/fakeout нет нигде

Вчерашний план: если real/fakeout-фильтр не в ленте (Angle 1 = AUC 0.50), то в L2-книге. Проверено и закрыто. Сначала подтвердил стену на полном 9-монет прогоне (март+апрель OOS): armed-cross **WR 19%, −0.086%/−0.093%, payoff ~2.0, раннеров (MFE≥1%) всего 5%**. Затем `icebreaker_book_gate.py` (FART/WIF/PEPE март, 1865 касаний, 0 без книги): реконструкция resting-стакана на `ts_cross` и `ts_cross−2с` (всё ts≤среза, без lookahead) → **void впереди / support-pull (снятие стены за миг до касания) / imbalance**. **AUC 0.44–0.53 ≈ 0.50 по всем 9 фичам** против real и win; gate-sweep по `depth_ratio` в обе стороны держит mean_net плоско −0.085%, WR 18–20%, payoff ~2.1 на любом отсечении. Глубины осмысленные (~$1M, qty-абсолют подтверждён) → настоящий null.

**(г) Итог:** на момент касания дискриминатора real/fakeout НЕТ ни в ленте, ни в стакане — 81% касаний фейк-поки, неотделимы в real-time.

**(д) Континуум подтверждения — DEAD, ледокол закрыт по входу окончательно** (`icebreaker_confirm_sweep.py`, 9 монет март, грид hold∈{0,2,5,10,30}с × Δ∈{0,10,20}bps, tape-only, все 9 монет оба месяца возможны). Вооружить уровень, войти taker-ом только если пробой удержал break-сторону `hold` секунд и стоит ≥Δ bps за уровнем (без lookahead). Подтверждение РЕАЛЬНО фильтрует фейки: WR 19→33–47%, kept good-rate 5.4→21%, drop-gr ~4–5% (раннеров оставляет). НО expectancy отрицателен в КАЖДОЙ ячейке (−0.067…−0.112%): стоп якорён к уровню, подтверждение требует ухода цены вверх (give-back 0.13–0.33%) → avgLoss раздувается, **payoff падает 2.04→1.0–1.8 быстрее, чем растёт WR**. Апрель OOS не нужен (из in-sample-минуса OOS-плюса нет). Потолок +0.33% требует входа НА уровне, узнать «пробой реален» можно только после ухода цены — недостижим без lookahead.

**(е) Angle 3 — динамика подхода + мультивариативный классификатор — DEAD** (`icebreaker_approach.py`, 3 монеты март, 1865 касаний; ответ на 3 вопроса юзера, которые снимки не закрывали). Книга на лесенке `ts_cross−{30,15,10,5,2,0}с`: (Q1) производная глубины/стены на подходе AUC 0.444–0.541 (шум); (Q3) «MM держит стену→фейд» — split на big-wall (n=622): снятие≥50% (n=30) good 0% vs устояла (n=592) good 4.2% vs база 4.0% = исход одинаков, стену снимают лишь 5% (footprint редкий И неинформативный); (Q2) **мультивариативный classifier** logreg+GBT 5-fold CV над всеми 22 фичами → **CV-AUC 0.49–0.51** vs good и win — никакая комбинация не разделяет. Решающий гвоздь: до пересечения фейк и реальный пробой микроструктурно идентичны, продолжение решает поток ПОСЛЕ касания — информация в будущем, не в пре-касательном прокси.

**ИТОГ ЛЕДОКОЛА (количественно):** дискриминатора real/fakeout на касании НЕТ нигде — лента (0.50), стакан-снимок (0.50), динамика подхода + мультивариатив (CV-AUC 0.50). Селекция реальна (favorable-excursion, WR), извлекаемого PnL нет — все честные механизмы входа (raw-touch, tape-gate, book-gate, confirm-continuum) ≤ безубытка. Единая суб-комиссионная/геометрическая стена убила все ветки. Новое: `icebreaker_{book_gate,confirm_sweep,approach}.py`, `ts_cross/side/level` в `levelcross`.

**Остался ручной шаг (на завтра):** сгенерён сбалансированный визуальный набор `icebreaker_eyeball.py` — **24 чарта (12 раннеров + 12 фейков, FART/WIF/PEPE), локально `data/charts/eyeball/`** (2 панели: цена+уровень+консолидация сверху, resting-стена $ + cumulative eaten $ у уровня снизу). На двух открытых — контраст «раннер: стену СНЯЛИ (→0, eaten≈0) / фейк: стену ЕЛИ (eaten растёт, рефиллы, абсорбция, разворот)»; это скалярный `cancel:eat`/`absorbed_frac` (Фаза 1), не робастный в агрегате — завтра смотрим глазами на весь набор, нет ли гештальта, который скаляр размывает. Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md) Фаза 2, [PLAN.md](PLAN.md) задача (ж).
...
1. **Vision реально «видит» (A):** AUC(score vs MFE≥1%) 0.550 март / **0.583 апрель** (реплицируется!); precision решения «trade» = 28.6% vs база 25.2% (март), **41.0% vs 28.4% (апрель)** — его пики в ~1.5× чаще раннеры, OOS усиливается. Дискреционный глаз существует.
2. **Но не конвертируется в PnL:** vision TRADE realized taker = **+0.005% март → +0.003% апрель**, и НЕ лучше отброшенного (SKIP +0.043/+0.011). Медиана плюсовая (+0.32…+0.34), среднее ноль → редкий полный стоп съедает.
3. **Выход не спасает (B,** `icebreaker_vision_exit.py`**):** ride-грид (лучший из 72 = оверфит в пользу гипотезы). vision TRADE/score≥7 taker: +0.032% март → **−0.019% апрель OOS** (coins+ 2/7); score≥6: +0.058 → +0.013 (схлопывается); ALL coins+ 7/9 → 2/9. Никакой runner-выход не выводит vision-срез в OOS-плюс.
4. **Vision-вердикт:** глаз реален, но эдж в favorable-excursion, не в PnL (стена 1.5/1.8). Инфра: lookahead-free render + **incremental dump/resume** (пережил жёсткий хост-стоп посреди прогона — VMware balloon/оверкоммит провайдера, тикет `docs/server_incident_2026-06-23_host_ticket.txt`).
5. **Декомпозиция PnL + контрфакт входа → КОРЕНЬ НАЙДЕН (`icebreaker_pnl_decomp.py`, `icebreaker_entry_cf.py`):** payoff≈1.0 (avgWin+0.47≈avgLoss−0.48), весь нетто на 10 худших трейдах (97% март/304% апр). Причина — **геометрия входа**: вход на close пробойной свечи = вершина спайка, стоп ~0.5%. Контрфакт «вход ПО УРОВНЮ» (идеал, fill-наивно): **payoff~2.7, WR~70%, +0.33%/сделку taker, хвост-устойчиво, OOS-реплицируемо** (март +0.335 / апрель +0.328). Геометрия даёт ×8 expectancy — селекция бессильна. Открыт **фронт Фаза 2: level-cross entry** (вооружить gated-уровень, войти taker на касании — fillable на раннерах, в отличие от пассива 1.8; риск — фейки без confirmation). Будущее: TIB (Лопез де Прадо) у пробоя — фильтр осведомлённого потока. Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md) Фаза 1.9.

### 2026-06-23 — Icebreaker Фаза 1.8: maker fill-risk → последний эдж умер, ледокол ЗАКРЫТ

Решающий гейт Фазы 1.7 (liquidity-vacuum maker-эдж +0.08–0.11%/сделку): переживает ли он честное лимитное исполнение. `icebreaker_maker_fill.py` (7 unit-тестов) ставит пассивный maker-лимит в момент закрытия пробоя, наливается **только на реальном касании ленты** в окне входа, гонит управляемую развязку от фактического fill.

1. **offset 0 (лимит на цене пробоя) воспроизводит +0.08–0.11%, но это не maker-ордер** — лимит маркетабелен (цена идёт сквозь него) = taker-вход по maker-фи. Прежнее число — артефакт допущения о гарантированном fill.
2. **Любой реально-пассивный лимит (≥5bps) ловит adverse selection** — на обоих месяцах (март+апрель) gr_miss ~2× gr_fill (37.5% vs 16% март; 23% vs 16% апрель): раннеры вакуум-пробоя убегают и не наливают пассив, наливают непропорционально фейлы.
3. **Реалистичный maker-entry/taker-exit = безубыток-до-минуса**, медиана отрицательна везде при offset>0, ≤4/9 монет. Полный ретест (level) — явный минус, 0–1/9.
4. **Вердикт:** единственный «живой» результат за всё исследование был артефактом исполнения → ледокол закрыт окончательно. Селекция вакуум-пробоя реальна (good-rate ×2 в favorable-excursion), но извлекаемого PnL под честным исполнением нет — та же субкомиссионная/исполнительная стена, что во всех ветках. Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md) Фаза 1.8.

### 2026-06-19 — Icebreaker (wall-eating): проверена и закрыта DEAD; построена инфра сбора L2

Заказная стратегия «поглощение стены» (плотность ≥$100k исчезла + OI растёт → вход по тренду, вход всегда на Bybit). Полный цикл проверки за 1 сессию:

1. **Квант-пасс их бэктеста** (264 трейда, +10.75%): breakeven 0.041%/сделку vs taker 0.11% → после комиссий −18%; эдж в 2 монетах из 35; DSR FAIL, MinBTL insufficient.
2. **Данные бесплатно:** Bybit отдаёт исторический L2 (ob200) + трейды (`quote-saver.bycsi.com`, `public.bybit.com/trading`) — покупка Tardis (~$200-600) не нужна. Скачали март-2026 (TAO/SIREN/ZEC/SUI) в **единый parquet-store** (8.8GB на серваке, переиспользуемо).
3. **Faithful-бэктест** (абсорбция через трейды + комиссии): все монеты net-отрицательны.
4. **Config sweep по обоим листам Excel** (важная самопроверка): на их грид-оптимуме TP1.2/SL0.2 у TAO **реальный gross +2.85%** — но эдж 0.049%/сделку < даже maker 0.06% → нетто-минус. Time-exit отрицателен (mean-reversion). Прежняя формулировка «анти-предсказателен» исправлена.
5. **Вердикт:** DEAD как нетто-прибыльная (эдж субкомиссионный). Их +10.75% — артефакт KuCoin lv50 + отсутствия комиссий.
6. **Инфра** (ветка `icebreaker-collector`, 99 тестов): universe/book/recorder/collector/live_bybit/bybit_archive + preflight/download/backtest/sweep скрипты. Live-коллектор проверен на серваке (~17 MB/символ/день). Детали: [ICEBREAKER_RESEARCH.md](ICEBREAKER_RESEARCH.md).

### 2026-06-14 — Weekend: найдена причина луз-стрика (порог входа), исправлено 3→4 of 5

Live 1 win / 7 (луз-стрик 6 подряд, 08-05…12-06, минусы в обе стороны). Разбор:

1. **Причина — не сигнал, а дрейф порога входа.** Прод (`ensemble.py`, `majority_threshold=3`)
   торговал при count ≥ 3 из 5 предикторов. Валидация требовала net ≥ 3, т.е. **4 из 5**.
   Три живых лузера (#2/#3/#6) — ровно сигналы 3-из-5, которые валид. правило пропустило бы.
2. **Ре-валидация (6/6 PASS):** 4of5 Sharpe 2.10 [CI 1.46-2.79] vs live 3of5 0.79 [CI −0.04-1.54,
   неотличим от нуля]. 150 маржинальных 3of5-сделок: total −22.6%, Sharpe −0.41 (отриц. эдж).
3. **Фикс:** `src/weekend/config.py` `majority_threshold` 3→4. Тесты 50/50 PASS.
4. **Две альтернативы отклонены бэктестом:** расширение окна удержания на дни (с SL строго хуже,
   Sharpe 2.10→1.0); гейт corr20(BTC,SPY)>0.55 (эффект перевёрнут — эдж сильнее при НИЗКОЙ
   корреляции, гейт даёт Sharpe 0.10).
5. **Сценарий #4 (22-05):** был реальный прокол −2% (low −2.34%) → выбит старым 2% SL, потом
   восстановление. Под текущим 5% катастрофным SL держался бы (+1.10%). Закрыто переходом на 5% SL.

Детали: [WEEKEND_THRESHOLD_RESEARCH](archive/WEEKEND_THRESHOLD_RESEARCH.md).
Скрипты: `weekend_threshold_revalidation.py`, `weekend_window_corr_backtest.py`.

⚠️ Правка порога в конфиге — **ещё не задеплоена** (ждёт коммита/пуша).

### 2026-06-13 — Weekend: исправлен сорванный live-вход (12 июня) + 3 бага

Сигнал 2026-06-12 (SHORT BTC, −3/5) **не открылся на бирже**, хотя в БД висел `status=open`. Разбор и фиксы:

1. **Главная причина — баланс < мин. ордера при 1x.** Мин. ордер 0.001 BTC (~$63) при `leverage=1` требует ~$63 маржи, на счетах ~$27–29 → обе биржи отклонили (`ab not enough` / `Margin insufficient`). Вернул `leverage=3` (commit 7360dfc). Важно: позиция всё равно зафиксирована на минимуме 0.001 BTC = ~$63 notional на биржу независимо от плеча — «3x» лишь даёт мелкому счёту выставить достаточно маржи, реальная экспозиция та же.
2. **Фантомный open.** `mark_entry()` ставил `status=open` ДО открытия на бирже → ретраи пропускали, settle/check-sl записали бы фейковый P&L. Теперь в live entry пишется только после реального филла (по средней цене), при полном фейле остаётся `pending` для ретраев (commit 7360dfc).
3. **Сироты conditional-ордеров.** `close_position` чистил ордера через обычный `fetch_open_orders`, не видящий Binance conditional (STOP_MARKET reduceOnly) → SL осиротевали после сеттла. Нашлась активная сирота от id=6 (BUY 64942, могла преждевременно закрыть текущий шорт). Теперь чистим оба бакета (`stop=True`) (commit 04afcb1).

**Итог:** позиция открыта вручную после деплоя — Bybit SHORT 0.001 @ 63417.6 + Binance SHORT 0.001 @ 63416.6, оба с SL @ 66591. БД id=7 = open. Дубли/сироты на Binance вычищены.

⚠️ **CI красный** (`test_funding_capture_flow::test_signal_emitted_with_correct_metadata` падает + `changes` job: `bad object` от shallow-clone) — авто-деплой заблокирован, все 3 фикса задеплоены ручным `git pull` на сервере. Чинить CI отдельно.
...
через N часов закрыть убыточную когда тренд определился. Убираем зависимость от prediction quality.

**Результат:** dead end. Все варианты хуже текущей продакшен стратегии.

Sharpe в таблице ниже — raw из бэктест-скрипта (упрощённая аннуализация). Реальный net Sharpe текущей стратегии = **1.42** OOS (см. запись 2026-04-29). Относительное сравнение корректно.

| Подход | H2 Sharpe (raw) | Avg PnL | WR | Cum |
|---|---|---|---|---|
| **B) Current (ensemble + flip 0.3%)** | **+0.89** | **+1.20%** | **62.5%** | **+57.5%** |
| C) Delta-neutral best (24h/0.2%) | +0.12 | +0.12% | 37.8% | +15.0% |
| D) Hybrid best (16h/0.3%) | +0.80 | +0.92% | 60.4% | +44.3% |

**Почему:**
1. 4 ноги комиссий ВСЕГДА (0.24%) vs 2.8 в среднем у текущей (0.17%)
2. Фиксация убытка на проигрышной ноге ≈ drawdown до checkpoint — BTC reversals часты
3. Без ensemble signal все 127 уикендов "tradeable" но avg trade ~0 (нет edge без prediction)
4. Ensemble vote (WR 60%+) реально даёт информацию — отказ от prediction = потеря edge

**Вывод:** "Подождать и посмотреть" хуже чем "предсказать и перевернуть если ошиблись".
Текущая B) = оптимальна (net Sharpe 1.42 OOS). Delta-neutral не компенсирует extra fees.

**Скрипт:** `scripts/tmp/weekend_delta_neutral.py`

### 2026-05-20 — Funding Capture: live review, обе биржи отрицательные

**Источник данных:** сервер (`engine_state.db`, `engine_state_binance.db`). Локальный `trades_log_server.json` устарел (201 vs 469+235 строк) — **всегда смотреть данные на сервере первым делом.**

**Bybit** (22 апр — 20 мая, трейды закончились ~11 мая, после — только skip):

| Неделя | Трейды | Avg PnL | Dollar | WR |
|---|---|---|---|---|
| W16 | 49 | -0.46% | -$4.50 | 16% |
| W17 | 26 | -0.39% | -$2.54 | 19% |
| W18 | 55 | -0.47% | -$6.37 | 18% |
| W19 | 6 | -0.28% | -$0.40 | 0% |
| **Total** | **136** | **-0.44%** | **-$13.81** | **17%** |

С 13 мая на Bybit 0 исполненных трейдов — все skip по thin_book (148 скипов total).

**Binance** (9-20 мая, 28 трейдов):

| Период | Трейды | Avg PnL | Dollar | WR |
|---|---|---|---|---|
| 9-10 мая | 10 | +0.13% | +$0.42 | 80% |
| 11+ мая | 18 | -0.24% | -$1.33 | 11% |
| **Total** | **28** | **-0.10%** | **-$0.91** | **36%** |

Binance лучше Bybit (avg -0.10% vs -0.44%, WR 36% vs 17%), но всё равно отрицательный.
Первые 2 дня на Binance были прибыльные (+$0.42, WR 80%), потом деградация.
Последние 5 трейдов — все в минус, включая монеты с rate 80-92 bps (HYPER, ENJ).

**Skip reasons (обе биржи):** thin_book=253, spread_too_wide=89, rate_dropped=4.
Фильтры отсекают >90% кандидатов, но те что проходят — всё равно убыточные.

**Вывод:** Funding capture = negative EV на текущем масштабе ($25-35 notional) на обеих биржах. Slippage систематически превышает funding income.
