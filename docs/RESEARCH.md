...
- **Icebreaker (Wall-Eating)** — **REVIVED → PAPER-READY (финальный раунд 2026-06-30)**. Был закрыт по всем фазам (стена/вход/выход суб-комиссионны, нет real/fakeout дискриминатора), НО финальный раунд нашёл живой **селекционный** эдж: момент пробойной свечи `mom-k0` (известен на входе, без lookahead) монотонно упорядочивает realized PnL; ось = **сила пробоя (mom-k0), коин-агностично** (не вола — vol-Pearson ~0; не атрибут монеты). Правило **CLEAN (spacing) + mom-k0 ≥ ~2.5% → вход маркетом → managed exit** на 2 годах × 40 монет (1m klines, haircut 5bps): **scorecard 7/7 при фикс-правиле** (CPCV Sharpe 1.84/P(>0) 96%, DSR p=0.001, PBO 0, multi-regime), net +0.112%/сделку, WR 61%; при n_trials=6 — 5/7 на волосок (3-й год закроет). Низкая частота (~13/мес). Статус: **paper-ready, не задеплоен, ПАУЗА** (отложены: 3-й год, tape-кросс-чек, paper). Актив: 8.8GB parquet-store (Bybit L2+trades) + 2-летние klines 40 монет + инфра. [Детали](ICEBREAKER_RESEARCH.md)
